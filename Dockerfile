...
RUN mkdir -p /data/devs/laravel-in-kubernetes /data/devs/laravel-in-kubernetes/bin

# Next, set our working directory
WORKDIR /data/devs/laravel-in-kubernetes
...
RUN addgroup -S composer \
    ...
    && chown -R composer /data/devs/laravel-in-kubernetes \
...
COPY --from=composer_base /data/devs/laravel-in-kubernetes /data/devs/laravel-in-kubernetes
...
COPY --from=composer_base /data/devs/laravel-in-kubernetes /data/devs/laravel-in-kubernetes
COPY --from=frontend /data/devs/laravel-in-kubernetes/public /data/devs/laravel-in-kubernetes/public
...
COPY --from=composer_base --chown=www-data /data/devs/laravel-in-kubernetes /data/devs/laravel-in-kubernetes
# COPY --from=composer_base --chown=root /data/devs/laravel-in-kubernetes /data/devs/laravel-in-kubernetes
COPY --from=frontend --chown=www-data /data/devs/laravel-in-kubernetes/public /data/devs/laravel-in-kubernetes/public
# COPY --from=frontend --chown=root /data/devs/laravel-in-kubernetes/public /data/devs/laravel-in-kubernetes/public

RUN chmod -R 775 /data/devs/laravel-in-kubernetes/storage
...
FROM nginx:1.25.4-alpine as web_server
...
RUN touch laravel.cron && \
    echo "* * * * * cd /data/devs/laravel-in-kubernetes && php artisan schedule:run" >> laravel.cron && \
